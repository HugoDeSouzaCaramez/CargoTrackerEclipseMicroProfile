...
public class Cargo {
    ...
    private void validateBookingCommand(BookCargoCommand command) {
        // Primeiro, precisamos determinar se a rota é intercontinental
        // Podemos fazer isso baseado nos códigos dos locais
        boolean isIntercontinental = isIntercontinentalRoute(
            command.getOriginLocation(), 
            command.getDestLocation()
        );
        
        // Agora podemos chamar o método com o parâmetro
        DeadlinePolicy policy = command.getDeadlinePolicy(isIntercontinental);

        if (!policy.isValidForBooking()) {
            throw new IllegalArgumentException(
                    String.format("Arrival deadline must be at least %d hours from now. Remaining: %d hours",
                            DeadlinePolicy.MIN_HOURS_FOR_BOOKING, policy.getRemainingHours()));
        }

        // Adiciona aviso para cargas urgentes (mas não bloqueia)
        if (policy.isUrgent()) {
            System.out.println(String.format(
                "[WARNING] Booking marked as %s - deadline within %d days",
    ...
    private boolean isIntercontinentalRoute(String originUnLocCode, String destUnLocCode) {
